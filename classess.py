...
class Lista_estudiantes:
    ...
    def delete_node_Espera(self, curso: Node_Curso):
            if not self.head_estudiante:
                return False
            else:
                current=self.head_estudiante
                self.head_estudiante=self.head_estudiante.next_estudiante
                return True
            return False

    def delete_node_Matriculados(self, value:str, curso:Node_Curso):
        if not self.head_estudiante:
            return False
        if self.head_estudiante.nombre_estudiante==value:
            current=self.head_estudiante
            self.head_estudiante=current.next_estudiante

        else:
            current=self.head_estudiante
            while current.next_estudiante:
                if current.next_estudiante.nombre_estudiante==value:
                    current.next_estudiante=current.next_estudiante.next_estudiante
                else:
                    return False
                current=current.next_estudiante
        self.matriculados=self.matriculados-1
    ...
    def getHead(self):
        if not self.head_estudiante:
            ...
            return self.head_estudiante.nombre_estudiante
